...
with open(file_to_save, "w") as txt_file:
    ...
    for cl in county_list:
        ...
        vote_county_percentage = float(cvotes) / float(total_votes) * 100
        # 6d: Print the county results to the terminal.
        candidate_results = (
            f"{county_name}: {vote_county_percentage:.1f}% ({cvotes:,})\n")
        print(candidate_results)
        # 6e: Save the county votes to a text file.
        txt_file.write(candidate_results)

        # 6f: Write an if statement to determine the winning county and get its vote count.
        if (cvotes > winning_county_count) and (vote_county_percentage > winning_county_percentage):
    ...
    winning_county_summary = (
        ...
        f"Largest Count Percentage: {winning_county_percentage:.1f}%\n"
        f"-------------------------\n")
